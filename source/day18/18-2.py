...
# Convert these
real_instructions = []
for junk, othajunk, code in instructions:
    count = int(code[1:6], 16)   # Convert from hex
    direction = code[-1]
    real_instructions.append((direction, count))
...
cells = []
cells.append(pos)
for direction, count in real_instructions:
    #for i in range(count):
    ccc[direction] += count
    pos = tupleadd(pos, nav[direction], mult=count)
    # cells.add(pos)
    cells.append(pos)
...
print(nrows, ncols, offset_rows, offset_cols)
raise
